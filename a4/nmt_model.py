...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #1. COnstruct tensor X with dimension (src_len, b, e)
        X = self.model_embeddings.source(source_padded)

        #2. apply the `pack_padded_sequence` function to X
        X = pack_padded_sequence(X, source_lengths)

        #3. Compute `enc_hiddens`, `last_hidden`, `last_cell` by applying the encoder to `X`.
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)

        #4. apply the `pad_packed_sequence` function to enc_hiddens
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)

        #5. reshape enc_hiddens from (src_len, b, h*2) to (b, src_len, h*2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        #6. init_decoder_hidden
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = alpha_t.view(alpha_t_view)
